...
하지만 FlexRay 는 기존의 CAN 대비 통신 방식이 달라 개발 및 검증에 대한 부담으로 시장에 널리 사용되지 못해 부품 값이 비싸다. CAN 과는 다르게 Clock 동기화를 할 수 있다. 시간을 완전히 예약할 수 있다.   
...
기존에는 각자가 다 알아서 했는데 슬슬 하나로 통합하고 싶음. 그래서 단계적으로 올려가는 중이다. 
