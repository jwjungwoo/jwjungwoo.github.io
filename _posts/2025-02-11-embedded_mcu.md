...
uint8 state = RED;
uint8 stop_increase_cnt = 0;

IFX_INTERRUPT(ISR0,0,0x10); // 0x10이 발생했을 때 ISR0를 실행한다.
IFX_INTERRUPT(STM_Int0Handler, 0, 100);

void ISR0(void) {
    stop_increase_cnt = !stop_increase_cnt;
}
...
void STM_Int0Handler(void)
{
    ...
    IfxStm_clearCompareFlag(g_Stm.stmSfr, g_Stm.stmConfig.comparator);
    IfxStm_increaseCompare(g_Stm.stmSfr, g_Stm.stmConfig.comparator, 50000000u); // 0 7개
    if (stop_increase_cnt == 0) {
        cnt++;
    }
    else {
        // do nothing
...
void initERU (void) {
    ...
    SCU_EICR1.U |= 1 << FENO_IDX; // 하강 엣지(신호가 HIGH에서 LOW로 변화할 때) 감지 활성화. 이 설정으로 신호가 하강 엣지에서 변할 때 인터럽트가 발생한다.
    // 떼는 동작은 rising egde
    SCU_EICR1.U |= 1 << EIENO_IDX; // 위에 하강 or 상승 엣지 결정 된거에서 그걸로 쓸건지 말건지. 비유를 들자면 falling or rising 은 어떤 총알을 쓸거냐고, EIENO는 총을 쏠거냐 말거냐임. 즉 해야한단 말씀.
